...
중요:
- 이 문서는 **Phase 2 full trace expansion**을 설명한다.
- 다만 Phase 1에서도 구현 저장소가 request 단위 복기 가능성을 self-check 할 수 있도록 reference contract로 선참고한다.
...
Phase 2에서 trace는 로컬 구현 이름과 무관하게 최소한 아래 의미를 재구성할 수 있어야 한다.
