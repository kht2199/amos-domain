...
## 작업 범위 원칙
- 이 `domain` 저장소 작업에서는 ontology / engine / phase / programs / roadmap 문서를 active source-of-truth로 다룬다.
...
- 이 저장소의 역할은 구현 저장소별 세부 구현안을 대신 정하는 것이 아니라, 각 저장소가 self-check 할 수 있도록 reference 자료, 읽는 순서, 점검 기준을 제공하는 것이다.
...
- 구현 저장소에 공통으로 안내할 우선 reference는 아래 문서들이다.
  ...
  - `docs/ontology/foundation.md`
  - `docs/ontology/request-interpretation.md`
  - `docs/engine/request-lifecycle.md`
...
- 위 3개 expansion engine 문서는 later expansion reference이며, foundation 범위에서는 self-check용 선참고 문서로 본다.
...
## 문서 정리 원칙
- `docs/ontology/foundation.md`는 도메인 객체/상태 기준을 우선 참조한다.
- `docs/ontology/request-interpretation.md`는 자연어 요청의 intent / target / scope / missing-slot 해석 축을 소유한다.
- `docs/engine/request-lifecycle.md`는 request-side runtime vocabulary와 상태 canonical을 소유한다.
- `docs/phase/foundation-engine.md`는 엔진 구조와 observability 관점을 담당한다.
