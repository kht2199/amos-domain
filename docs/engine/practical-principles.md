...
> 이 문서는 AMOS 반도체/운영 모델의 active 실전 원칙 문서다.
...
- supervisor / trace / policy-permission 문서는 later expansion reference다.
- canonical 목록 자체를 이 문서에서 다시 정의하지 않는다.

## 1. 문서보다 런타임 정합성을 우선한다
- README나 guide의 표현과 실제 코드 의미가 다르면 코드를 기준으로 문서를 즉시 맞춘다.
- ontology 문서는 예쁜 정의집이 아니라 runtime, state, trace, policy와 실제로 연결되어야 한다.
...
## 2. 질문 해석보다 안전한 전환을 우선한다
- 잘못된 친절한 답변보다 안전한 clarify 전환이 낫다.
- `failed`가 발생했는데도 억지로 다음 실행으로 넘어가지 않는다.
- action 계열은 승인 없이 자동 실행하지 않는다.

## 3. Foundation은 최소 의미 체계에 집중한다
- 도메인 객체/상태 canonical은 `../ontology/foundation.md`를 따른다.
- 자연어 요청 해석 축은 `../ontology/request-interpretation.md`를 따른다.
- request-side runtime vocabulary와 상태 canonical은 `request-lifecycle.md`를 따른다.
- trace / policy / supervisor 상세 vocabulary는 전용 engine 문서가 소유한다.
- 이 문서는 위 기준을 실제 운영에서 어떻게 지킬지 원칙만 남긴다.
...
연결 지점이 없으면 아직 active canon으로 올리지 않는다.
